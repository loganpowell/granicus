...
const LC_keys = obj => Object.entries(obj).reduce((a, [k, v]) => {
    ...
    a[lowercase(k)] =
        ending === "AT" && v
            ...
            : ending !== "ID" && typeof Number(v) === "number"
                ? Number(v)
                : v;
    return a;
}, {});
...
const partition = ({ subscriber_id = "", destination = "", latitude = 0, longitude = 0, referrer_url = "", subscriber_created_at = "", subscriber_source = "", subscription_created_at = "", subscription_added, subscription_source = "", topic_id = "", }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const subDone = yield grafetch({
        ...
        if (r.errors) {
            ...
            return grafetch({
                ...
                if ((!r.data.getSub.deleted_at && is_deletion) ||
                    (!r.data.getSub.created_at && !is_deletion)) {
                    log(`${r.data.getSub.id} getSub: updating ${is_deletion ? "deleted_at" : "created_at"}`);
                    return grafetch({
                        operation: updateSub,
                        vars_obj: subInput,
                    }).then(r => {
                        log(`${r.data.updateSub.id} updateSub: ${is_deletion ? "deleted_at" : "created_at"} UPDATED`);
                        return;
                    });
                }
    ...
    return Promise.all(done).then(r => {
